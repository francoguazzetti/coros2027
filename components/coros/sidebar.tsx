  ...
  return (
    <aside className="flex h-full w-[200px] flex-col border-r border-border bg-background">
      ...
      <div className="flex items-center justify-between py-4 px-4 pr-0">
        <Link href="/projects" className="text-sm font-bold text-foreground hover:text-foreground/80 transition-all duration-200 hover:scale-105 active:scale-95 inline-block">
          Coro
        </Link>
        ...
        {navItems.map((item, index) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "py-1 text-sm text-foreground transition-all duration-200 hover:text-foreground/80 hover:scale-105 active:scale-95 inline-block",
                item.indent && "pl-4",
